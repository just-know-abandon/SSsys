...
@Controller
@WebServlet(name = "UpdateBaoxiulistService", urlPatterns={"/api/UpdateBaoxiulistService"})
public class UpdateBaoxiulistService extends HttpServlet {
    @Autowired
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (parameter.equals("1")) {
                ...
                baoxiulist.setId(Integer.parseInt(baoxiulistId));
                baoxiulist.setBaoxiuxinxi(baoxiulistxiuxinxi);
                baoxiulist.setSushehao(baoxiulistSushehao);
                baoxiulistService.updateByKey(baoxiulist);
                code = 200;
            } else {
                //参数验证失败
                code = 300;
