...
@Controller
@WebServlet(name = "DeleteShiwulistService", urlPatterns={"/api/DeleteShiwulistService"})
public class DeleteShiwulistService extends HttpServlet {
    @Autowired
    private ShiwulistService shiwulistService;

    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        PrintWriter out = response.getWriter();
        String shiwulistId = request.getParameter("shiwulistId");
        List<String> strList1 = new ArrayList();
        List<String> strList2 = new ArrayList();
        JsonArray dataJson = new JsonArray();
        JsonObject result = new JsonObject();
        strList1.add(shiwulistId);
        strList2.add("shiwulistId");
        String parameter = "";
        int code = 0;//状态码
        ...
        try {
            ...
            if (parameter.equals("1")) {
                ShiwulistKey shiwulistKey=new ShiwulistKey();
                shiwulistKey.setId(Integer.parseInt(shiwulistId));
                shiwulistService.deleteByKey(shiwulistKey);
                code = 200;
            } else {
                //参数验证失败
                code = 300;
