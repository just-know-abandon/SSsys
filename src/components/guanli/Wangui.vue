<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>晚归登记</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- Card卡片 -->
    <el-card>
      <!-- 搜索与添加区域 -->
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-button type="primary" @click="addDialogVisible = true">晚归登记</el-button>
        </el-col>
      </el-row>
      <!-- 用户列表区域(表格) -->
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column prop="time" label="日期时间"></el-table-column>
...
export default {
  data () {
    return {
      ...
      addFormRules: {
        ...
        name: [
          { required: true, message: '请输入姓名信息', trigger: 'blur' },
          { min: 2, max: 10, message: '长度在 2 到 10 个字符', trigger: 'blur' }
        ],
        time: [
          { required: true, message: '请输入晚归时间', trigger: 'blur' },
  ...
  methods: {
    getData () {
      this.$api.QueryWanguilistService().then(res => {
        console.log(res)
        if (res.code === 200) {
          const _table = []
          res.data.forEach(item => {
            const tableitem = {
              id: item.wanguilistId,
              sushehao: item.wanguilistSushehao,
              name: item.wanguilistName,
              time: item.wanguilistRiqi
            }
    ...
    // 点击按钮添加新用户
    addUser () {
      this.$api.InsertWangguilistService({
        wanguilistId: this.addForm.id,
        wanguilistSushehao: this.addForm.sushehao,
        wanguilistName: this.addForm.name,
        wanguilistRiqi: this.addForm.time
      }).then(res => {
        console.log(res)
        if (res.code !== 200) {
    ...
    // 修改用户信息并提交
    editUserInfo (id) {
      this.$api.UpdateWanguilistService({
        wanguilistId: this.editForm.id,
        wanguilistName: this.editForm.name,
        wanguilistSushehao: this.editForm.sushehao,
        wanguilistRiqi: this.editForm.time
      }).then(res => {
        console.log(res)
        if (res.code !== 200) {
    ...
    async removeUserById (id) {
      console.log(id)
      // const wanguilistId = id
      // this.$api.DeleteWanguilistService({
      //   wanguilistId: id
      // }).then(res => {
      //   console.log(res)
      //   if (res.code !== 200) {
    ...
    search () {
      this.$api.QueryLikeWanguilistService({
