<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>失物登记</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- Card卡片 -->
    <el-card>
      <!-- 搜索与添加区域 -->
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-button type="primary" @click="addDialogVisible = true">失物登记</el-button>
        </el-col>
      </el-row>
      <!-- 用户列表区域(表格) -->
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column prop="shiwu" label="失物信息"></el-table-column>
    ...
    <el-dialog
      ...
      label-width="100px">
        ...
        <el-form-item label="失物信息" prop="shiwu">
          <el-input v-model="addForm.shiwu"></el-input>
        </el-form-item>
      </el-form>
      <!-- 底部区域 -->
    ...
    <el-dialog
      ...
      label-width="100px">
        ...
        <el-form-item label="失物信息" prop="shiwu">
          <el-input v-model="editForm.shiwu"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  data () {
    return {
      ...
      addFormRules: {
        ...
        shiwu: [
          { required: true, message: '请输入失物信息', trigger: 'blur' },
  ...
  methods: {
    getData () {
      this.$api.QueryShiwulistService().then(res => {
        console.log(res)
        if (res.code === 200) {
          const _table = []
          res.data.forEach(item => {
            const tableitem = {
              id: item.shiwulistId,
              sushehao: item.shiwulistSushehao,
              shiwu: item.shiwulistxiuxinxi
            }
    ...
    // 点击按钮添加新用户
    addUser () {
      this.$api.InsertShiwulistService({
        shiwulistId: this.addForm.id,
        shiwulistSushehao: this.addForm.sushehao,
        shiwulistxiuxinxi: this.addForm.shiwu
      }).then(res => {
        console.log(res)
        if (res.code !== 200) {
    ...
    // 修改用户信息并提交
    editUserInfo (id) {
      this.$api.UpdateShiwulistService({
        shiwulistId: this.editForm.id,
        shiwulistxiuxinxi: this.editForm.shiwu,
        shiwulistSushehao: this.editForm.sushehao
      }).then(res => {
        console.log(res)
        if (res.code !== 200) {
    ...
    async removeUserById (id) {
      ...
      if (confirmResult !== 'confirm') {
        ...
        this.$api.DeleteShiwulistService({
          shiwulistId: id
        }).then(res => {
          console.log(res)
          if (res.code !== 200) {
    ...
    search () {
      this.$api.QueryLikeShiwulistService({
