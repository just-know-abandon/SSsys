  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column prop="renshu" label="人数"></el-table-column>
        <el-table-column prop="shezhang" label="舍长"></el-table-column>
    ...
    <el-dialog
      ...
      label-width="70px">
        ...
        <el-form-item label="人数" prop="renshu">
          <el-input v-model="addForm.renshu"></el-input>
        </el-form-item>
        <el-form-item label="舍长" prop="shezhang">
          <el-input v-model="addForm.shezhang"></el-input>
        </el-form-item>
    ...
    <el-dialog
      ...
      label-width="70px">
        ...
        <el-form-item label="人数" prop="renshu">
          <el-input v-model="editForm.renshu"></el-input>
        </el-form-item>
        <el-form-item label="舍长" prop="shezhang">
          <el-input v-model="editForm.shezhang"></el-input>
        </el-form-item>
        <el-form-item label="电话" prop="phone">
...
export default {
  data () {
    ...
    return {
      ...
      addFormRules: {
        ...
        renshu: [
          { required: true, message: '请输入宿舍人数', trigger: 'blur' },
          { min: 1, max: 2, message: '长度在 2 到 15 个字符', trigger: 'blur' }
        ],
        shezhang: [
          { required: true, message: '请输入舍长名', trigger: 'blur' },
  ...
  methods: {
    getData () {
      this.$api.QuerySushelistService().then(res => {
        console.log(res)
        if (res.code === 200) {
          const _table = []
          res.data.forEach(item => {
            const tableitem = {
              id: item.sushelistId,
              sushehao: item.sushelistSushehao,
              renshu: item.sushelistRenshu,
              phone: item.sushelistPhone,
              shezhang: item.sushelistShezhang
            }
    ...
    // 点击按钮添加新用户
    addUser () {
      this.$api.InsertSushelistService({
        sushelistId: this.addForm.id,
        sushelistSushehao: this.addForm.sushehao,
        sushelistRenshu: this.addForm.renshu,
        sushelistPhone: this.addForm.phone,
        sushelistShezhang: this.addForm.shezhang
      }).then(res => {
        console.log(res)
        if (res.code !== 200) {
    ...
    // 修改用户信息并提交
    editUserInfo (id) {
      this.$api.UpdateSushelistService({
        sushelistId: this.editForm.id,
        sushelistPhone: this.editForm.phone,
        sushelistSushehao: this.editForm.sushehao,
        sushelistShezhang: this.editForm.shezhang,
        sushelistRenshu: this.editForm.renshu
      }).then(res => {
        console.log(res)
        if (res.code !== 200) {
          return this.$message.error('修改用户信息失败')
    ...
    async removeUserById (id) {
      ...
      if (confirmResult !== 'confirm') {
        ...
        this.$api.DeleteSushelistService({
          sushelistId: id
        }).then(res => {
          console.log(res)
          if (res.code !== 200) {
    ...
    search () {
      this.$api.QueryLikeSushelistService({
